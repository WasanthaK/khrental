...
--
-- The UPDATE and CHECK-constraint statements use dynamic SQL deliberately:
-- SQL Server compiles a batch before executing ALTER TABLE, so a newly added
-- column cannot be referenced later in the same static batch on first run.
...
BEGIN TRY
    ...
    BEGIN
        THROW 50001, 'dbo.tenant_memberships does not exist.', 1;
    END;
